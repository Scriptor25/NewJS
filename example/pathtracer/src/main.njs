...
let ground_sphere = make<sphere>(sphere.create([ 0, -1000, 0 ], 1000, ground_material.base))
hittable_list.add(world, ground_sphere.base)
...
for (let a = -11; a < 11; ++a) {
    for (let b = -11; b < 11; ++b) {
        ...
        if (math.length(center - [ 4, 0.2, 0 ]:point3) > 0.9) {
            let sphere_material: shared_ptr<material>
            if (choose_mat < 0.8) {
                // diffuse
            ...
            let sphere = make<sphere>(sphere.create(center, 0.2, sphere_material.base))
            hittable_list.add(world, sphere.base)
        }
    }
}

let material1 = make<dielectric>(dielectric.create([ 1.0, 1.0, 1.0 ], 1.5))
let sphere1 = make<sphere>(sphere.create([ 0, 1, 0 ], 1.0, material1.base))
hittable_list.add(world, sphere1.base)

let material2 = make<lambertian>(lambertian.create([ 0.4, 0.2, 0.1 ]))
let sphere2 = make<sphere>(sphere.create([ -4, 1, 0 ], 1.0, material2.base))
hittable_list.add(world, sphere2.base)

let material3 = make<metal>(metal.create([ 0.7, 0.6, 0.5 ], 0.0))
let sphere3 = make<sphere>(sphere.create([ 4, 1, 0 ], 1.0, material3.base))
hittable_list.add(world, sphere3.base)
